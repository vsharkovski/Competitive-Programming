...
ll F(int food, int room, int capacity) {
  if (food == 0) return room == K-1;
  if (room == K) return 0;
  if (dp[food][room][capacity] != -1) return dp[food][room][capacity];
  int res = 0;
  if (capacity == 0)
    return dp[food][room][capacity] = F(food, room+1, M);
  else if (capacity == M)
    return dp[food][room][capacity] = F(food-1, room, capacity-1);
  else
    return dp[food][room][capacity] = F(food-1, room, capacity-1) + F(food, room+1, M);
}
