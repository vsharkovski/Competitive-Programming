...
void Solve() {
  ...
  ll compleft = n, submleft = k;
  while (compleft > 0 && submleft > 0 && submleft >= compleft) {
    ll removethisturn = submleft / compleft; //maksimalno
    for (int i = 0; i < n && compleft > 0 && submleft > 0; ++i) {
      if (v[i] > 0) {
        ll toremove = min(v[i], removethisturn);
        submleft -= toremove;
        v[i] -= toremove;
        if (v[i] == 0) --compleft;
      }
  ...
  if (submleft > 0 && compleft > 0 && submleft < compleft) {
    //posledna tura; submleft < compleft ==> normalno simuliraj
    for (int i = 0; i < n && compleft > 0 && submleft > 0; ++i) {
      if (v[i] > 0) {
        submleft -= 1;
        v[i] -= 1;
