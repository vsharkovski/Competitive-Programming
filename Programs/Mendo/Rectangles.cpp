...
int f(int i, int prevheight) {
  if (i == n) return 0;
  if (dp[i][prevheight] != -1) return dp[i][prevheight];
  
  int o1 = abs(a[i] - prevheight) + b[i] + f(i+1, a[i]);
  if (i == 0) o1 -= a[i];
  
  int o2 = abs(b[i] - prevheight) + a[i] + f(i+1, b[i]);
  if (i == 0) o2 -= b[i];

  return dp[i][prevheight] = max(o1, o2);
}
