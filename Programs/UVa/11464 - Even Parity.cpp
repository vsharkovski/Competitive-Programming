...
// passes every test case on uDebug but gets WA on the site
// if you can prove it's incorrect feel free to tell me

int n;
bool grid[16][16];
int best;

inline int odd(int r, int c) {
  return ((r > 0 && grid[r-1][c]) + (r < n && grid[r+1][c]) + (c > 0 && grid[r][c-1]) + (c < n && grid[r][c+1])) % 2;
}
