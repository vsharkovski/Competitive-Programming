...
void Solve() {
  ...
  vi r(n+1), bad(n+1, 0), numsmaller(n+1);
  vii v;
  ...
  while (k--) {
    ...
    cin >> x >> y;
    if (r[x] > r[y]) ++bad[x];
    else if (r[y] > r[x]) ++bad[y];
  }
  
  sort(all(v), [&](ii &x, ii &y) {
    return x.first < y.first;
  });
  
  numsmaller[v[0].second] = 0;
  for (int i = 1; i < n; ++i) {
    numsmaller[v[i].second] = numsmaller[v[i-1].second];
    if (v[i-1].first < v[i].first) {
      int cnt = 1, j = i-2;
      while (j >= 0 && v[j].first == v[j+1].first) ++cnt, --j;
      numsmaller[v[i].second] += cnt;
    }
  }
  
  for (int i = 1; i <= n; ++i) {
    cout << numsmaller[i] - bad[i] << " ";
  }
  cout << endl;
