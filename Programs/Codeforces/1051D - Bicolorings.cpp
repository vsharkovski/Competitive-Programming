...
ll dp[1001][2001][5];
// index, br cc, prev top row and bottom row

ll ways(int i, int m, int p) {
  if (i == n) return m == k;
  if (m > k) return 0;
  ...
  if (p == 0b00) {
    res += ways(i+1, m,   0b00);
    res += ways(i+1, m+1, 0b01);
    res += ways(i+1, m+1, 0b10);
    res += ways(i+1, m+1, 0b11);
  } else if (p == 0b01) {
    res += ways(i+1, m,   0b00);
    res += ways(i+1, m,   0b01);
    res += ways(i+1, m+2, 0b10);
    res += ways(i+1, m,   0b11);
  } else if (p == 0b10) {
    res += ways(i+1, m,   0b00);
    res += ways(i+1, m+2, 0b01);
    res += ways(i+1, m,   0b10);
    res += ways(i+1, m,   0b11);
  } else if (p == 0b11) {
    res += ways(i+1, m+1, 0b00);
    res += ways(i+1, m+1, 0b01);
    res += ways(i+1, m+1, 0b10);
    res += ways(i+1, m,   0b11);
  }
  return dp[i][m][p] = res % mod;
}

void Solve() {
  cin >> n >> k;
  memset(dp, -1, sizeof(dp));
  ll ans = ways(1, 1, 0b00) + ways(1, 2, 0b01) + ways(1, 2, 0b10) + ways(1, 1, 0b11);
  ans %= mod;
  cout << ans << endl;
